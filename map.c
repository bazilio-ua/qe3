...
void Map_LoadFile (char *filename)
{
	...
	if (!*ValueForKey(world_entity, "wad"))
	{
		Sys_Printf ("WARNING: no \"wad\" key\n");
	}
	else
	{
		strcpy(wadkey, ValueForKey(world_entity, "wad"));
	...
	if (ent)
	{
		...
		GetVectorForKey (ent, "origin", g_qeglobals.d_xyz.origin);
		g_qeglobals.d_camera.angles[YAW] = FloatForKey (ent, "angle");
	}
	else
	{
		g_qeglobals.d_camera.angles[YAW] = 0;
		VectorCopy (vec3_origin, g_qeglobals.d_camera.origin);
		VectorCopy (vec3_origin, g_qeglobals.d_xyz.origin);
	}
...
void Map_New (void)
{
	...
	SetKeyValue (world_entity, "wad", wadstring);
...
void Map_RegionXY (void)
{
	...
	region_mins[0] = g_qeglobals.d_xyz.origin[0] - 0.5*g_qeglobals.d_xyz.width/g_qeglobals.d_xyz.scale;
	region_maxs[0] = g_qeglobals.d_xyz.origin[0] + 0.5*g_qeglobals.d_xyz.width/g_qeglobals.d_xyz.scale;
	region_mins[1] = g_qeglobals.d_xyz.origin[1] - 0.5*g_qeglobals.d_xyz.height/g_qeglobals.d_xyz.scale;
	region_maxs[1] = g_qeglobals.d_xyz.origin[1] + 0.5*g_qeglobals.d_xyz.height/g_qeglobals.d_xyz.scale;
	region_mins[2] = -4096;
	region_maxs[2] = 4096;
