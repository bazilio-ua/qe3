...
/* EER1
	hwndEnt[EntList] = CreateWindow ("listbox", NULL, 
		LBS_STANDARD | LBS_NOINTEGRALHEIGHT | LBS_WANTKEYBOARDINPUT | WS_BORDER
...
*/
	hwndEnt[EntList] = CreateWindowEx(WS_EX_CLIENTEDGE, 
		"listbox", 
		NULL, 
...
/* EER1
	hwndEnt[EntProps] = CreateWindow ("listbox", NULL, 
		LBS_STANDARD | LBS_NOINTEGRALHEIGHT | LBS_USETABSTOPS | WS_BORDER
		| WS_VSCROLL | WS_CHILD | WS_VISIBLE,
		5, 100, 180, 99,
		g_qeglobals.d_hwndEntity,
		(void *)IDC_E_PROPS,
	...
	if (!hwndEnt[EntProps])
		Error ("CreateWindow failed");
*/
	hwndEnt[EntProps] = CreateWindowEx(WS_EX_CLIENTEDGE, 
		"listbox", 
		NULL, 
...
/* EER1
	...
	if (!hwndEnt[EntComment])
		Error ("CreateWindow failed");
*/
	hwndEnt[EntComment] = CreateWindowEx(WS_EX_CLIENTEDGE, 
		"edit", 
		NULL, 
...
/* EER1
	...
	if (!hwndEnt[EntKeyField])
		Error ("CreateWindow failed");
*/
	hwndEnt[EntKeyField] = CreateWindowEx(WS_EX_CLIENTEDGE, 
		"RichEdit", 
		NULL, 
...
/* EER1
	...
	if (!hwndEnt[EntValueField])
		Error ("CreateWindow failed");
*/
	hwndEnt[EntValueField] = CreateWindowEx(WS_EX_CLIENTEDGE, 
		"RichEdit", 
		NULL, 
		ES_AUTOHSCROLL | WS_CHILD | WS_TABSTOP | WS_VISIBLE,
		5, 100, 180, 99,
	...
	if (!hwndEnt[EntValueField])
		Error("CreateWindowEx failed");
/* EER1
	g_qeglobals.d_hwndConsole = CreateWindow ("edit", NULL, 
		ES_MULTILINE | ES_READONLY | WS_VSCROLL | WS_CHILD | WS_VISIBLE,
		5, 100, 180, 99,
		g_qeglobals.d_hwndEntity,
		(void *)IDC_E_STATUS,
...
*/
	...
	g_qeglobals.d_hwndTexture = WTex_Create ();
...
void CreateEntityWindow(HINSTANCE hInstance)
{
	...
	hwnd = CreateDialog(hInstance, (char *)IDD_ENTITY, g_qeglobals.d_hwndMain, (DLGPROC)NULL);
	if (!hwnd)
		Error ("CreateDialog failed");

	GetEntityControls (hwnd);
	DestroyWindow (hwnd);
...
void SizeEntityDlg(int iWidth, int iHeight)
{
	...
	MOVE(g_qeglobals.d_hwndConsole,	DlgXBorder, DlgYBorder, iWidth - (2 * DlgXBorder), iHeight - (2 * DlgYBorder) );
